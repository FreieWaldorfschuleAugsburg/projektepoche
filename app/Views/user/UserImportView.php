            ...
            <div class="card-body">
                ...
                <?php if (isset($users)): ?>
                    <form method="post" action="<?= base_url('user/import') ?>">
                        <div class="d-flex justify-content-between align-items- py-2">
                            ...
                            <button type="submit" name="intent" value="confirm" class="btn btn-success"><?= lang('user.import.save') ?></button>
                        </div>
                                ...
                                <?php foreach ($users as $user): ?>
                                    <tr>
                                        <td id="td-id-<?= $user->getId() ?>" class="td-class-<?= $user->getId() ?>"
                                            data-title="<?= $user->getName() ?>"><input class="form-control" name="username[]" id="username[]"
                                                                                        value="<?= $user->getName() ?>">
                                        </td>
                                        <td><input name="password[]" class="form-control" value="<?= $user->getPassword() ?>">
                                        </td>
                ...
                <div class="mt-2">
                    <form action="<?= base_url('user/import') ?>" method="post" enctype="multipart/form-data">
                        ...
                        <button type="submit" name="intent" value="import" class="btn btn-primary"><?= lang('user.import.button') ?></button>
                    </form>
                </div>
            </div>
        </div>
